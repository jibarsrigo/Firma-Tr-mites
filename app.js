...
VERSION 1.3.70  - Panel Reglas: catálogo con las N reglas de acciones.json (una línea por id; sin agrupar).

VERSION 1.3.71  - error_clave_movil: Acción Qué pasa/Qué hacer también en overrides con Firma KO.
...
VERSION 1.3.78  - Fase firma: no cierra con solo TR_SGO; hace falta TR_RGI (multi-firma / KO tras OK).

VERSION 1.3.79  - Acción: conservar líneas en blanco (aire entre pasos); cadena Qué pasa «no confiar».

VERSION 1.3.80  - SGI sin cierre: desempate por TR_CAR (CERTIFICADO→entorno); sin pistas Cl@ve móvil en entorno.

VERSION 1.3.81  - Linux en SistraHelp → siempre Android (retira uso de error_autofirma_cliente_linux).

VERSION 1.3.82  - SAF_27 + método Cl@ve marcado: aviso en flujo/Acción (no es Cl@ve; es certificado/Autofirm@).

VERSION 1.3.83  - Cartel SAF_27 (intento).
VERSION 1.3.84  - Cartel SAF_27: Autofirma — selección de certificado no correcto al firmar.

VERSION 1.3.85  - Aviso «método Cl@ve pero es Autofirm@» siempre primera línea de Qué pasa (cliente/genérico/SAF_27).

VERSION 1.3.86  - Pista dispositivo: literal «Autofirma o del Cliente de Firma Móvil» (Android habitual).

VERSION 1.3.87  - Cl@ve: si hay algún 103-15 en la traza, manda sobre 8-15 (no depende del orden de pegado).

VERSION 1.3.88  - Acción {lit}: escapar HTML del literal (registro presentador / Portafib).

VERSION 1.3.89  - Literales: índice O(n) al emparejar línea/original (evita indexOf cuadrático).

VERSION 1.3.90  - Traza mixta: aviso suave si varios TR_INI con el mismo ID de expediente.

VERSION 1.3.91  - Quita console.log de depuración en el análisis; html v1.3.9 (charset al inicio).
...
// 👉 Cl@ve marcado + regla Autofirma: aviso SIEMPRE como primera línea de Qué pasa
//    (después de intros fitxer buit / cancelada / etc., se reubica al inicio).
function textoAvisoMetodoClaveConAutofirma(idRegla) {
  if (idRegla === "error_autofirma_servidor") {
    return "Aviso: el método marcado es Cl@ve, pero este fallo no es de Cl@ve. "
      + "SAF_27 es de certificado local / Autofirm@. Orientar como Autofirma (selector de certificado, instalación), no como códigos Cl@ve.";
  }
  return "Aviso: el método marcado es Cl@ve, pero este fallo no es de Cl@ve. "
    + "La traza apunta a certificado local / Autofirm@. Marcar Certificado + sistema y volver a analizar.";
}

function asegurarAvisoMetodoClaveConAutofirmaPrimero(textoAccion, idRegla, marcadoClaveSinCert) {
  if (!marcadoClaveSinCert || !idRegla || idRegla.indexOf("error_autofirma") !== 0) {
    return textoAccion;
  }
  const aviso = textoAvisoMetodoClaveConAutofirma(idRegla);
  let t = String(textoAccion || "");
  // Quitar avisos previos / frase estática del JSON para no duplicar ni dejarlos abajo
  t = t.replace(/\r\n/g, "\n").replace(
    /^.*(?:Aviso:\s*el m[eé]todo marcado es Cl@ve|Acceso marcado como Cl@ve)[^\n]*\n?/gim,
    ""
  );
  if (/Qu[eé]\s*pasa\s*:/i.test(t)) {
    return t.replace(/((?:<b>)?Qu[eé]\s*pasa\s*:)\s*/i, "$1\n" + aviso + "\n");
...
function aplicarIntroAccionAutofirmaCliente(textoAccion, lineas, idRegla) {
  ...
  if (esEscritorio) {
    ...
    if (hayTimeout && hayServInter) {
      ...
      partes.push(
        "Pista de dispositivo: el error «Autofirma o del Cliente de Firma Móvil» suele verse en Android, "
        + "pero también puede ser iPhone u otro entorno: confirmar SO en SistraHelp (doble clic Inicio/Carga)."
      );
    }
  }
...
function detectarSoEscritorioDesdePistas(soIniCar, soSinKo) {
  ...
  // Linux no se trata como escritorio real: en CAU es Android mal reportado
  if (so === "linux") return "android";
  if (so === "mac" || so === "windows") return so;
  return null;
}
...
//    CA: "S'ha firmat amb un certificat on el nif associat és X, però es requeria el nif Y"
//    ES: "Se ha firmado con un certificado donde el NIF asociado es X, pero se requería el NIF Y"
//    Variante real: «nif associado» (doble s). Causa típica: selector de certificado equivocado.
function esErrorNifCertificadoNoCoincideHelper(linea) {
  const s = String(linea || "");
  const mencionaNifCert =
    /NIF\s+ASS?OCIA(?:T|DO|DA)\b/i.test(s) ||
    /FIRMAT AMB UN CERTIFICAT ON EL NIF/i.test(s) ||
    /FIRMADO CON UN CERTIFICADO DONDE EL NIF/i.test(s);
  const mencionaRequerido =
    /REQUER[IÍ]?A?\s+EL\s+NIF/i.test(s) ||
    /SE\s+REQUER[IÍ]A\s+EL\s+NIF/i.test(s);
  return mencionaNifCert && mencionaRequerido;
}
...
function extraerNifsCertificadoNoCoincide(linea) {
  ...
  const mCert = s.match(new RegExp("NIF\\s+ASS?OCIA(?:T|DO|DA)\\s*[ÉE]?S?\\s*" + idDoc, "i"));
  const mReq = s.match(new RegExp("(?:SE\\s+)?REQUER[IÍ]?A?\\s*EL\\s*NIF\\s*" + idDoc, "i"));
  return {
...
// Varios TR_INI misma fecha/hora = duplicado al pegar (no avisar).
// Varios TR_INI en momentos distintos + mismo ID expediente = mismo trámite retomado → aviso suave.
// Varios IDs de expediente = mezcla real → aviso fuerte.
function detectarTrazaMultiTramite(lineasTraza) {
  ...
  const instanciasIni = lineasIni.map(linea => {
    ...
    return {
      ...
      idExpediente: c?.idExpediente || null,
  ...
  const idsExpediente = new Set();
  const clavesExpediente = new Set();
  for (const linea of lineasTraza || []) {
    if (!/^TR_[A-Z]+\s+-/.test(linea)) continue;
    const c = parseCamposEventoTraza(linea);
    if (c?.idExpediente) idsExpediente.add(c.idExpediente);
    const clave = claveInstanciaTramite(c);
    if (clave && c?.idExpediente) clavesExpediente.add(clave);
  }

  const idsIniDistintos = [...new Set(iniMomentosDistintos.map(i => i.idExpediente).filter(Boolean))];
  const clavesIniDistintas = [...new Set(iniMomentosDistintos.map(i => i.clave).filter(Boolean))];
  const variosExpedientes =
    idsExpediente.size > 1 || idsIniDistintos.length > 1;

  const motivos = [];
  let severidad = null; // "fuerte" | "suave"

  if (iniMomentosDistintos.length > 1 && variosExpedientes) {
    severidad = "fuerte";
    motivos.push(
    ...
    if (idsIniDistintos.length > 1) {
      motivos.push(idsIniDistintos.length + " identificadores de expediente distintos en esos TR_INI.");
    } else if (clavesIniDistintas.length > 1) {
      motivos.push(clavesIniDistintas.length + " identificadores distintos en esos TR_INI.");
    }
  } else if (variosExpedientes) {
    severidad = "fuerte";
    motivos.push(idsExpediente.size + " identificadores de expediente distintos en la traza.");
  } else if (iniMomentosDistintos.length > 1) {
    severidad = "suave";
    motivos.push(
      iniMomentosDistintos.length + " × TR_INI en fechas u horas distintas, con el mismo ID de expediente: "
      + "suele ser el mismo trámite retomado (p. ej. otro día), no una mezcla por búsqueda de DNI."
    );
  }

  return {
    esMulti: severidad != null,
    severidad,
    motivos,
...
function htmlAvisoTrazaMultiTramite(info) {
  ...
  const suave = info.severidad === "suave";
  const motivos = info.motivos.map(m => "<li>" + escHtmlAviso(m) + "</li>").join("");
  const borde = suave ? "#d4a017" : "#c0392b";
  const fondo = suave ? "#fff8e6" : "#fdecea";
  const color = suave ? "#7a5c00" : "#922b21";
  const titulo = suave
    ? "⚠ Varios accesos — mismo expediente"
    : "⚠ Traza mixta — varios trámites";
  const intro = suave
    ? "<p style=\"margin:0 0 8px;\">Hay más de un <b>Inicio trámite (TR_INI)</b> en momentos distintos, pero el <b>ID de expediente es el mismo</b>. "
      + "Suele ser el ciudadano retomando el mismo trámite; el análisis puede seguir siendo útil. "
      + "Si dudáis, pegad solo los eventos del día / intento que os interesa.</p>"
  ...
  const pie = suave
    ? "<p style=\"margin:12px 0 0;font-size:13px;color:#666;\">No es obligatorio volver a pegar; revisad que el veredicto corresponda al intento actual.</p>"
  ...
  return "<div class=\"panel-card\" style=\"margin-top:22px;border:2px solid " + borde + ";\">"
    + "<div class=\"panel-card__head\" style=\"text-transform:none;background:" + fondo + ";color:" + color + ";\">" + titulo + "</div>"
...
function htmlAvisoTrazaMultiTramiteCompacto(info) {
  ...
  const borde = suave ? "#d4a017" : "#c0392b";
  const color = suave ? "#7a5c00" : "#922b21";
  const etiqueta = suave ? "Varios TR_INI (mismo expediente)" : "Traza mixta";
  const cola = suave
    ? " — suele ser el mismo trámite retomado; revisad el intento que os interesa."
    : " — el veredicto puede ser incorrecto. Pegar solo la traza del ID de trámite objetivo.";
  return "<div style=\"margin-bottom:12px;padding:10px 12px;background:" + fondo + ";border:1px solid " + borde + ";border-radius:8px;font-size:13px;color:" + color + ";\">"
    + "<b>⚠ " + etiqueta + ":</b> " + escHtmlAviso(info.motivos[0] || "Varios trámites en la traza")
    + cola
    + "</div>";
}
...
const versionHtmlEl = document.getElementById("versionHTML");
if (versionHtmlEl) {
  versionHtmlEl.innerText =
    "html " + (typeof VERSION_HTML !== "undefined" ? VERSION_HTML : "—");
}
...
<ul>
  ...
  <li><b>Traza mixta:</b> IDs de expediente distintos → aviso rojo (posible búsqueda por DNI). Varios TR_INI mismo ID → aviso ámbar (mismo trámite retomado). Duplicados TR_INI mismo momento no avisan.</li>
...
const GRUPOS_CATALOGO_REGLAS = [
  { titulo: "A) Antes de firmar (pre-firma)", ids: ["fallo_formulario", "fallo_portafib"] },
  {
    ...
    ids: [
      "error_clave_8_15", "error_clave_101", "error_clave_103", "error_clave_103_15",
      "error_clave_104", "error_clave_firma_cancelada", "error_clave_movil",
      "error_clave_movil_no_permitida", "error_firma_fitxers_500"
    ]
  },
  {
...
/** Catálogo HTML: una línea por cada regla de acciones.json (sin agrupar ids). */
function htmlCatalogoReglasDesdeAcciones() {
  const lista = (accionesJSON && Array.isArray(accionesJSON.acciones)) ? accionesJSON.acciones : [];
  const porId = {};
  lista.forEach(a => { if (a && a.id) porId[a.id] = a; });
  const n = lista.length;
  const legacy = { error_autofirma: true, error_fire: true };
  let html = "<li class=\"reglas-titulo\">3. Catálogo: las " + n + " reglas de acciones.json (una por línea; mismo id que usa el motor)</li>";
  const vistos = new Set();

  function lineaRegla(id) {
    const marca = legacy[id] ? "⚙" : "✔";
    const desc = DESCRIPCION_REGLA_CATALOGO[id] || "Ver acción en acciones.json.";
    return "<li>" + marca + " <b>" + id + "</b> — " + desc + "</li>";
  }

  GRUPOS_CATALOGO_REGLAS.forEach((grupo) => {
    html += "<li class=\"reglas-titulo\">" + grupo.titulo + "</li>";
  ...
  const extras = lista.filter(a => a.id && !vistos.has(a.id));
  if (extras.length) {
    html += "<li class=\"reglas-titulo\">E) Otras (en acciones.json, fuera de grupos)</li>";
    extras.forEach(a => { html += lineaRegla(a.id); });
  }

  html += "<li class=\"reglas-sep\" aria-hidden=\"true\"></li>";
  html += "<li><i>Total: " + n + " reglas</i> · ✔ activa · ⚙ reserva/legacy</li>";
  return html;
}
...
btnTabla.onclick = (e) => {
  ...
  const vJson = accionesJSON?.version || "—";
  const nReglas = (accionesJSON?.acciones || []).length;
  abrirPanelAyuda(TITULO_PANEL_REGLAS, `
<ul>
  <li class="reglas-titulo">Guía del motor de análisis (js v ${VERSION_JS} · acciones.json v ${vJson} · ${nReglas} reglas)</li>
  ...
  <li class="reglas-titulo">6. Autofirma cliente — capas (para no liar SO y fallo)</li>
  ...
  <li class="reglas-titulo">7. Pruebas y versiones</li>
  ...
  <li class="reglas-titulo">Pendiente — continuar en otro momento</li>
  <li>Migrar a Qué pasa/Qué hacer el resto de acciones (lista en Info y en la regla Cursor acciones-que-pasa-que-hacer) · literales más limpios · mails Autofirma por SO · aviso KO previos si el trámite acaba OK.</li>
...
const lineasTraza = [];
const indicesLineasTraza = [];
for (let i = 0; i < lineas.length; i++) {
  if (esLineaFormatoTrazaSistraHelp(lineas[i])) {
    lineasTraza.push(lineas[i]);
    indicesLineasTraza.push(i);
  }
}
...
//    La fase de firma NO se cierra solo con TR_SGO (puede haber varias firmas).
//    Se da por cerrada al pasar a registro: TR_RGI (o TR_REG / TR_FIN).
...
if ((hayRGI || hayREG || hayFIN) && !hayKoTrasUltimaFirmaOk) {
  // Inicio registro o cierre administrativo, sin KO posterior al último Firma OK
...
else if (haySGO && !hayRGI && !hayREG && !hayFIN && !hayKoTrasUltimaFirmaOk) {
  // 👉 Hay Firma OK pero aún no hay TR_RGI: fase de firma abierta (multi-firma / pendiente registro).
  //    Se usa firma_ok solo para mostrar la regla informativa firma_correcta (texto: no dar por cerrada).
  contexto.fase = "firma_ok";
}
else {
  // 👉 fallback seguridad
...
const lineasErrorEntries = lineasTraza
  .map((linea, j) => {
    const indice = indicesLineasTraza[j];
...
// 👉 Código Cl@ve: primera línea ERROR+CLAVEFIRMA (orden SistraHelp: lo reciente arriba).
// 🔹 Si en toda la traza hay algún 103-15, manda sobre 8-15 (regla CAU; no depende del orden de pegado).
...
const esCodigoClave815 =
  !!codigoClaveDetectado && /^(8|9|10|11|12|13|14|15)$/.test(codigoClaveDetectado);

if (esCodigoClave815 || !codigoClaveDetectado) {
  for (let i = 0; i < lineasTraza.length; i++) {
    const linea = lineasTraza[i];
    if (!linea.includes("CLAVEFIRMA") || !linea.includes("ERROR")) continue;
    ...
    if (matchCodigo && matchCodigo[1] === "103" && matchTipus && matchTipus[1] === "15") {
      lineaErrorClave = linea;
      codigoClaveDetectado = "103";
      tipusResultatDetectado = "15";
      break;
    }
  }
}
...
const hayAutofirmaKoPrevio = lineasTraza.some(linea => {
  if (!/^TR_SGX\s+-/i.test(linea)) return false;
  if (/CL@VEFIRM@|CLAVEFIRMA|PROVE[IÏ]DOR:\s*CLAVEFIRMA/i.test(linea)) return false;
  return /AUTOFIRM@/i.test(linea) ||
    /SERVIDOR INTERMEDI|CLIENTE DE FIRMA M[ÒOÓ]?VIL|CLIENT DE FIRMA/i.test(linea) ||
    /SIGNATURA CANCEL|FIRMA CANCEL/i.test(linea);
});
...
// 🔹 Códigos 8–15 → error Cl@ve individual
  ...
  // 👉 Solo TR_SGI sin TR_SGX/TR_SGO: Cl@ve móvil o entorno FIRE/Autofirma.
  //    Desempate: TR_CAR/INI (CERTIFICADO → entorno; CLAVE_MOVIL → móvil);
  //    si CAR es permanente/desconocido → selector del técnico.
  idReglaDetectada = resolverReglaSgiSinCierre(lineasTraza, esCert, esClave);
...
if (!haySGI) {
  ...
  if (hayKoTrasUltimaFirmaOk) {
    frase += " Había una Firma OK previa: un solo OK no cierra el trámite si hace falta más de una firma o si el reintento posterior falla.";
  }
  fraseDiagnostico = frase;
  ...
  cartelDiagnostico = cartelAzul("Autofirma — selección de certificado no correcto al firmar");
  ...
  if (esClave && !esCert) {
    fraseDiagnostico += " No es un error de Cl@ve: el KO es de certificado local / Autofirm@ (SAF_27), "
      + "aunque el método marcado sea Cl@ve.";
  }
  ...
  // Nota carpeta/Cl@ve móvil solo si el acceso en traza es Cl@ve móvil (no si TR_CAR dice certificado).
  if (hayPatronSgiSinCierre && hayAccesoClaveMovilEnTraza) {
  ...
  cartelDiagnostico = cartelAzul(hayRGI ? "Firma OK" : "Firma OK (fase abierta)");
  if (!hayRGI) {
    fraseDiagnostico = "Aparece Firma OK (TR_SGO), pero no hay "
      + literalFlujo("TR_RGI — Inici registre") + ". "
      + "La fase de firma no se da por terminada solo con un OK: el trámite puede exigir varias firmas o aún no ha pasado a registro.";
  } else {
    fraseDiagnostico = "Aparece Firma OK (TR_SGO) e Inicio registro (TR_RGI) en la traza.";
    if (!hayFIN && !hayREG) {
...
// =====================================
// 🔴 ACCIÓN RECOMENDADA / CONTROL POR REGLA
// =====================================
...
if (accionData && accionData.accion) {
  ...
  if (avisoMultiTramite.esMulti) {
    if (avisoMultiTramite.severidad === "suave") {
      textoAccion = "⚠ Varios TR_INI (mismo expediente): suele ser el mismo trámite retomado en días distintos. "
        + "Revisad que el veredicto corresponda al intento que os interesa.\n\n"
        + textoAccion;
    } else {
  ...
  const literalGris = (texto, cursiva) =>
    "<span style=\"color:#9a9890;font-size:12px" + (cursiva ? ";font-style:italic" : "") + "\">\""
  ...
  // 👉 NIF manda, pero también hay literal de cadena en la traza (poco habitual; no inventar si no está)
  if (idReglaDetectada === "error_certificado_nif_no_coincide" && hayErrorCadenaCertificacion) {
    textoAccion = insertarBloqueEnQuePasaAccion(
      textoAccion,
      "También aparece en la traza error de cadena de certificación (InvalidCertificateChain) en algún intento. "
        + "Lo que manda ahora es el otro NIF; si tras elegir el certificado correcto sigue fallando por cadena, tratar esa casuística (poco habitual)."
    );
  }
  ...
  if (textoAccion.indexOf("{lit}") !== -1) {
    ...
    if (hayErrorClaveReal) {
      const resumenClave = textoResumenCodigoClaveDetectado(codigoClaveDetectado, tipusResultatDetectado);
      notasPrevias.push(
        "Hubo Firma KO previos de Cl@ve" +
        (resumenClave ? " (código " + resumenClave + ")" : "") +
        ". El ciudadano recuperó y completó el trámite."
      );
    }
    const haySgoAutofirma = lineasTraza.some(l =>
      /^TR_SGO\s+-/i.test(l) && /AUTOFIRM@/i.test(l)
    );
    if (haySgoAutofirma && (hayErrorClaveReal || hayMetodoFirmaClaveEnKo)) {
      notasPrevias.push(
        "La Firma OK final es con Autofirm@ (certificado local), tras los intentos fallidos con Cl@veFirm@."
      );
    }
    if (hayClaveMovilNoPermitida) {
      notasPrevias.push(
        "También aparecen ERROR de acceso: Cl@ve Móvil no está permitida en este trámite (intentos previos). No impidió el cierre."
      );
    }
    ...
    hayClaveMovilNoPermitida
  ) {
    // Firma KO con código manda; el ERROR de acceso CLAVE_MOVIL se explica en Qué pasa (no sustituye la regla).
    let textoCodigoAccion = "el código Cl@ve de esta Acción";
    if (idReglaDetectada === "error_clave_8_15") textoCodigoAccion = "un error de clave 8-15";
    else if (idReglaDetectada === "error_clave_101") textoCodigoAccion = "un error de clave 101";
    else if (idReglaDetectada === "error_clave_103_15") textoCodigoAccion = "un error de clave 103-15";
    else if (idReglaDetectada === "error_clave_103") textoCodigoAccion = "un error de clave 103";
    else if (idReglaDetectada === "error_clave_104") textoCodigoAccion = "un error de clave 104";
    textoAccion = insertarBloqueEnQuePasaAccion(
      textoAccion,
      "Más tarde en la traza aparece ERROR de acceso: Cl@ve Móvil no está permitida en este trámite "
        + "(Probable acceso con clave movil desde carpeta ciudadana). "
        + "Si se usa clave movil para cargar tramite desde carpeta, puede no aparecer selector de método de firma y dar error. "
        + "Debe usar Cl@ve Permanente o certificado para cargar trámite desde carpeta ciudadana. "
        + "En este caso avisamos de como proceder ante " + textoCodigoAccion + "."
    );
  } else if (idReglaDetectada === "error_autofirma_cliente_generico" && hayMetodoFirmaAutofirmaEnKo && haySignaturaCancelada) {
    // Matiz «firma cancelada»; el aviso Cl@ve↔Autofirm@ se pone al final como 1.ª línea de Qué pasa.
    ...
    if (haySignaturaCancelada && !hayErrorClaveReal) {
      textoAccion =
        "Qué pasa:\n"
        + "La firma se inicia pero falla con Signatura cancel·lada, sin código de error Cl@ve (8–15, 101, 103…).\n"
        + "Puede ser Cl@ve móvil; o que la ventana de elegir certificado no cargue, se bloquee o el usuario la cierre; o acceso con nivel insuficiente.\n\n"
        + "Qué hacer:\n"
        + "1. Revisar en SistraHelp el último acceso al trámite (doble clic en Inicio trámite o Carga): si figura CLAVE_MOVIL.\n"
        + "2. Si el acceso fue con certificado, valorar entorno Autofirma/FIRE.\n"
        + "3. Enviar el mail al ciudadano.";
    } else {
      textoAccion =
        "Qué pasa:\n"
        + "La firma se inicia pero falla en Cl@ve sin código de error reconocido en la traza (aparece Firma KO).\n\n"
        + "Qué hacer:\n"
        + "1. Revisar en SistraHelp el último acceso al trámite (doble clic en Inicio trámite o Carga): si figura CLAVE_MOVIL.\n"
        + "2. Enviar el mail al ciudadano.\n"
  ...
  textoAccion = asegurarAvisoMetodoClaveConAutofirmaPrimero(
    textoAccion,
    idReglaDetectada,
    esClave && !esCert
  );
  ...
  const flushBloqueAccion = () => {
    ...
    // Conservar líneas en blanco → aire entre pasos (antes se filtraban y todo quedaba apelotonado)
    const body = bloqueActual.lineas.map(linea => {
      if (!String(linea || "").trim()) {
        return "<span class=\"accion-linea-vacia\" aria-hidden=\"true\"></span>";
      }
      return lineaAccionAHtml(linea);
    }).join("<br>");
...
// 👉 Primer TR_CAR/INI con método reconocible (SistraHelp: lo más reciente arriba).
function detectarAccesoRecienteEnTraza(lineas) {
  const arr = Array.isArray(lineas) ? lineas : [];
  for (let i = 0; i < arr.length; i++) {
    const linea = arr[i];
    if (!/^TR_INI\s+-/.test(linea) && !/^TR_CAR\s+-/.test(linea)) continue;
    const acceso = detectarAccesoEnLineaIniCar(linea);
    if (acceso && acceso !== "desconocido") return acceso;
  }
  return "desconocido";
}

// 👉 Solo TR_SGI sin KO/OK: CERTIFICADO en CAR → entorno; CLAVE_MOVIL → móvil;
//    permanente/desconocido → selector (Certificado = FIRE/entorno).
function resolverReglaSgiSinCierre(lineasTraza, esCert, esClave) {
  const accesoCar = detectarAccesoRecienteEnTraza(lineasTraza);
  if (accesoCar === "certificado") return "error_autofirma_entorno";
  if (accesoCar === "clave_movil") return "error_clave_movil";
  if (esCert && !esClave) return "error_autofirma_entorno";
  return "error_clave_movil";
}
...
function clasificarTipoKoLinea(linea) {
  ...
  if (/FITXER SIGNAT.*BUIT|FICHIERO SIGNAT.*VAC|PLUGIN.*AUTOFIRMA|SIGNAT EST[AÁ] BUIT/i.test(l)) {
    return "fitxer_buit";
  }
  ...
  if (/ERROR DE AUTOFIRMA|ERROR D['']AUTOFIRMA|AUTOFIRMA|AI500001/i.test(l)
...
function clasificarResultadoVentana(lineasVentana) {
  ...
  if (/SAF_27|SAF27\b/i.test(textoVentana)) {
    ...
    } else if (/FITXER SIGNAT.*BUIT|FICHIERO SIGNAT.*VAC|PLUGIN.*AUTOFIRMA|SIGNAT EST[AÁ] BUIT/i.test(textoVentana)) {
    ...
    } else if (/ERROR DE AUTOFIRMA|ERROR D['']AUTOFIRMA|AUTOFIRMA|AI500001/i.test(textoVentana) || metodoFirma === "autofirma") {
...
function htmlAccesoFlujoFirma(acceso, opts) {
  ...
  // Con acceso certificado ya conocido, no añadir «Posible Cl@ve móvil…» (contradice el CAR).
  const mostrarPistaAmbiguia = pistaSinCierre && acceso !== "certificado";
